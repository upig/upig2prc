...
;$dlgMain = GUICreate("upig2Lrf (v8.10.30.19) 将Txt批量转换到Lrf (sony prs 505)", 631, 443, 193, 115, -1, 0x00000018)
#Region ### START Koda GUI section ### Form=d:\3_appdata\microsoft\desktop\upig2lrf\新版\ui.kxf
$dlgMain = GUICreate("upig2Lrf (9.2.25.24) 将Txt批量转换到Lrf (sony prs 505)", 640, 465, 193, 115, -1, 0x00000018)
...
;"upig2lrf工具用于将中文txt文件批量转换为lrf文件" & @CRLF & @CRLF & "请将需要转换的文件或文件夹拖进来" & @CRLF & "支持多个文件，文件夹一起拖" & @CRLF &"只支持遍历一级文件夹"&@CRLF&@CRLF &@CRLF &@CRLF &@CRLF &@CRLF & "注意:运行本软件先得安装Calibre(http://calibre.kovidgoyal.net)" & @CRLF & "说明:文件转换出错或出现乱码后可尝试修改GBK等参数" & @CRLF & "联系:有什么Bug欢迎反馈给我(31531640@qq.com)"
...
If $g_reg ==-1 Then
	RegWrite("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2lrf转换\command", "", "REG_SZ", @ScriptFullPath&" ""%1""")
	RegWrite("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2lrf转换\", "", "REG_SZ", "用&upig2lrf转换")
	RegWrite("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2lrf转换\command", "", "REG_SZ", @ScriptFullPath&" ""%1""")
	RegWrite("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2lrf转换\", "", "REG_SZ", "用&upig2lrf转换")
	$g_reg = 1
EndIf

If $g_reg ==0 Then
	RegDelete("HKEY_CLASSES_ROOT\txtfile\shell\A用&upig2lrf转换\")
	RegDelete("HKEY_CLASSES_ROOT\Folder\shell\A用&upig2lrf转换\")
EndIf
...
Func ConvertTxt2MarkDown($oldTxt, $newTxt, $oldTxtTitle)
	...
	While 1
		...
		If $g_bInsertTOC==True Then			
			...
			if UBound($strFindArr)<>0 Then
				...
				If $g_bEngilsh==False Then
					;如果没有章，　也没有第
					If StringInStr($strFindArr[0], "第")==0 and  StringInStr($strFindArr[0], "章")==0 Then
						$bTitle=0
					EndIf				
		...
		if $ChapterCharacterCount>$g_chptMaxChrcters Then 
			$ChapterNo+=1
			$line&=@CRLF&"第"&$ChapterNo&"章"&@CRLF&"-------------"&@CRLF
			$ChapterCharacterCount = 0
			$bTitle = 1
		EndIf
		...
		If $g_bInsertTOC==True and $ChapterNo==0 and $bTitle==0 Then
			$ChapterNo=1
			If $g_bEngilsh==False Then
				FileWriteLine($outFp, "正文开始"&@CRLF&"--------"&@CRLF)	
			Else
